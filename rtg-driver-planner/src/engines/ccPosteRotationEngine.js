  ...
  _ensureCascade(targetIso, state, teams) {
    ...
    while (cursor.getTime() <= targetDate.getTime()) {
      ...
      Object.keys(byBlock).forEach(key => {
        ...
        // conservé dans chaque groupe. Le "poste d'hier" retenu, par ordre de
        // priorité :
        //   1. une correction ad hoc (case cliquable) — toujours une vraie
        //      décision, quelle que soit sa date ;
        //   2. une correction d'import dont la date cible est déjà PASSÉE au
        //      moment de la lecture — déjà réelle, confirmée après coup ;
        //   3. à défaut, pour un jour déjà passé (hier < aujourd'hui) SANS
        //      aucune correction : la valeur AUTO calculée (_dayZone) — elle
        //      est déjà présentée comme registre historique de ce jour-là à
        //      l'affichage (PlanningEngine.generateDailyAssignments, jours
        //      < aujourd'hui) ; s'y fier ici aussi évite une incohérence
        //      entre ce qui est affiché et ce que l'escalier calcule (cas
        //      réel GR HADDAZI : CHAKIR/BOUTALBI/EL MENYARI/HAIOUED, sans
        //      aucune correction le 25/09, mais bien affichés au quai ce
        //      jour-là) ;
        //   4. sinon (aujourd'hui/futur, sans correction confirmée) : rien
        //      n'est encore décidé — jamais deviné à l'avance (demande
        //      explicite de l'exploitant, §"PAR DÉFAUT L'ORDRE DOIT RESTER
        //      LE MÊME... POUR TOUS LES PROCHAINS JOURS") — une prévision
        //      d'import pour un jour pas encore réel (ex. GR
        //      HADDAZI/TIMSADAK, poste "DTV" prérempli pour un jour alors
        //      futur) ne compte donc pas non plus.
        // Seule la NATURE de la zone (poste physique ou PARC/AUTORISE, cf.
        // CC_NON_PHYSICAL_ZONES) détermine "était au quai" — jamais son
        // libellé exact : une correction marquant un conducteur "AUTORISE"
        // (jamais un poste physique) ne compte donc jamais comme quai (cas
        // réel GR HADDAZI/ELMANSORI).
